...
- In an ingestion-v2 structured workspace, `answer_explanation_mode` is independent from processing/artifact mode and safely defaults to `ordinary`. Both routes run `study_guide_author.py prepare`, fill the fixed annotations, require one detailed beginner-first explanation per item, persist notebooks, compile, create/attach/verify claims, and import the canonical full Guide. In `ordinary`, the annotation itself contains the explanation with `ai_supplement` provenance; never call a second Provider or claim isolation. `isolated` is an off-by-default, full-v2-only extension with two-stage consent: after local host probing, Provider/API-billing and retention/privacy disclosure, a no-upload planning opt-in permits persisting the mode and preparing packet/annotations/requests/plan; only after inspecting the plan's exact item/image scope and call count, separately checking current official pricing, disclosing a bounded estimate, and accepting that exact `plan_id` may the host make calls. Each fresh/stateless tool-disabled invocation sees only the fixed question, answer, target language, and target-scoped assets, returns `answer_explanation` plus non-rendered `coverage`, and is imported with a separate host-owned receipt. A model family, subscription, API key, `full`, or `visual` never enables this extension. Target-scoped means `target_item_only`, or prompt-only `target_with_required_context` with exact sorted `required_context_ids`; answer assets remain target-only. Packet, annotations, notebook bindings, manifest, rendering and QA all bind the chosen mode. A language/mode/fact/asset change makes the chain stale; only `isolated` reruns the external per-item receipt chain. New v2 Guides omit generic self-check panels. A hand-written complete v2 Guide draft is a no-Python-only, unverified fallback. Ingestion-v1 remains read-only and cannot claim current v2 gates.
