...
3. **故意放几条越界探针**（`answerable:false`）：学生可能会问、但你的材料根本没讲的问题
   ...
   它直接对应 skill 宣称的"物理防幻觉"。**优先出 near-miss 越界题**（话题贴着材料、但材料里没有
   确切答案的题，如材料讲了某算法却问它没提的一个参数），而非明显跑题的——明显越界太好答，
   near-miss 才分得出"真读了材料并知道边界"与"看着相关就编"。详见 [`docs/judge-calibration.md`](../docs/judge-calibration.md)。
