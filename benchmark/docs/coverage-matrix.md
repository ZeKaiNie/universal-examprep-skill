...
- **Tier 2 现有确定性 mock 层（🟡）**：`scenarios.json` 共 **16 个行为场景**，其中 **15 个在确定性 `--mock` 里被断言、进 CI、零成本**，1 个（`lazy_load_best_effort`）标了 `best_effort:true`、确定性模式下报 `SKIP` 不断言（需 transcript/真 LLM 才能验）。15 个断言场景 = 原 7 个（quiz_bank-only / 来源标注 / 提示·跳过·错题归档 / 疑难追踪 / 断点恢复 / 无 Python 降级 / 0 基础精讲已升级为七步）+ A 线新增 5 个（`visual_first_assets` 题面图门禁 / `scope_override` 越界覆盖声明 / `teaching_template` 七步模板+来源块 / `time_budget_no_questions` ≤1天严禁提问 / `knowledge_window_recheck` 窗口外复核）+ A8b 新增 1 个（`language_first_ask` 语言首问合并）+ v4 红线新增 2 个（`notebook_persist_ok` 教学落盘回执 / `workspace_confirm_ok` 建区必确认）。清单见 [`../behavior_smoke/README.md`](../behavior_smoke/README.md)。
