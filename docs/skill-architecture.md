...
- full 的 `step_by_step` 只改变教学清单的每轮题数；`teaching_example_roster_exhausted` 不是章节完成，不能替代 Guide、题库、typed-unit、资产、checkpoint 或 phase 门禁。`no_questions=true` 或 lightweight 只让已存 step 偏好 dormant 并使用有效 `batch`，不覆写偏好。每个 `teaching_baseline.json` ID 都必须在当前 `teaching_examples.json` 有同章快照；quiz-only 同 ID 不可替代。逐题 binding 一旦建立，切回 batch 也继续接受 notebook/manifest live 校验。
- 图结构题先运行确定性算法，再渲染。
...
- `answer_explanation_mode` 与 processing/artifact mode 正交且默认 `ordinary`。两种模式都要求每题详细、初学者可读的答案解释；ordinary 在正常 annotations 中生成并禁止 isolation/provider receipt。isolated 仅用于 full-v2，并分两次同意：说明 Provider/API 独立计费和保留/隐私边界后，只授权不上传的本地规划；看到准确 item/image 范围、调用数并按当前官方价格取得有假设的估算后，才授权绑定 exact plan 的逐题 fresh/stateless 调用及强回执链。模型系列、订阅、API Key、`full` 或 `visual` 都不构成启用证据。
