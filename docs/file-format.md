...
独立。新建、缺失、legacy 或非法值的存储层安全回退仍为 `ordinary`。`ordinary` 要求
ingestion-v2 Guide 的每题都有详细、零基础友好的 `answer_explanation`，但解释直接写在
mode-bound annotations 中，不带隔离回执；`isolated` 才启用逐题请求、coverage、host
receipt 与最终 contract。进入 full-v2 Guide 时由宿主做能力握手：只有能证明每题启动全新
独立子上下文，并能把输入和工具限制到准确单题请求时，才在用户未退出的情况下默认写入
`isolated`，同时只提示一次会额外消耗当前宿主的模型额度/时间；该原生路线不需要第二把
API Key，也不需要外部上传同意。能力缺失、继承或无法确认时保持 `ordinary`。另行计费的
外部 Provider 只是用户明确点名后的备用路线，仍采取两阶段同意：不上传的准确规划，以及
核对逐题/图片范围、调用数、当前价格和保留/隐私边界后的 exact-plan 上传同意。模型系列、
订阅、API Key、`full` 或 `visual` 本身既不能证明隔离能力，也不能授权外部上传。
