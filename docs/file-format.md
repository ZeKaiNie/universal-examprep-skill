...
- `teaching_examples.json` 仅为可达性清单，不是第二答案源；无独立答案但有完整演示的 Example 可不进 bank。项含唯一 `id`、chapter/phase、`paired_problem|worked_example`、题答来源与 assets；ID 可与 bank 重叠。quiz/teaching/notebook/Guide 的规范 ID 共用安全 Unicode 规则：长度 1–200，禁止空白、``[]#|`/\``、Unicode `Cc/Cf/Cs`、replacement character 与 noncharacter。
- tutor 只用 `list_teaching_examples.py --workspace <ws> --chapter <N> --json` 惰性读取当前章。只有完整路线、`no_questions=false` 且规范 `preferences.interaction_style=step_by_step` 时才可加 `--next-pending`。该基线早于显式 processing selector，所以缺少 `processing_mode` 等于历史隐式 full；显式非 `full` 值使偏好 dormant。选择器在一个 workspace validation lock 中读取 manifest/state/baseline/notebook，要求 `<N>` 等于 `current_phase`，按 manifest 顺序返回第一项无有效证据的题，并报告 total/completed/pending/next 与 `teaching_example_roster_exhausted`。两个 binding 不能复用一个 `notebook_ref`。只有缺失文件/条目和 anchor/marker/hash/revision 漂移可作为有界 stale 诊断重新进入 pending；parent/leaf reparse、非目录/非普通文件、realpath escape、非法 UTF-8、未闭 fenced block、parse/block 结构损坏、schema、重复及 unexpected evidence 均失败关闭。
- `teaching_baseline.json` 是 `policy` 精确等于 `append_only` 的保留事实：每个 ID 必须在 `teaching_examples.json` 中保有同一 canonical chapter 的当前快照；quiz-bank-only 副本不构成 teaching roster。ID 缓存消失、跨章漂移、逐章映射与全集不一致均 fail-loud；不得手改/清零。legacy 无该文件时才读 `ingest_report.json.teaching_example_ids`。
...
- `teaching_example_bindings`（可选）: 精确字段对象数组 `{id, notebook_ref, notebook_block_sha256, manifest_item_sha256}`。未绑定的 `teaching_examples` 字符串仍是合法 batch/legacy 历史；一旦 ID 有 binding，live marker/type/ID/anchor、notebook 块哈希、manifest item 哈希及对应 notebook evidence 在节奏切换后仍必须全部匹配。
...
单题节奏必须先用 `notebook.py add-entry --type walkthrough --teaching-example` 写完整七步 walkthrough，再把其真实 anchor 交给 `record-taught-example --id <id> --notebook-ref <path#anchor>`；若 ID 本身生成空 Markdown slug，必须提供可生成非空 anchor 的描述性 title。后者在同一锁/保存中校验 effective step、当前第一 pending、manifest、walkthrough 类型、ID 与保留 marker，并同时写普通 evidence 和上述 binding。step-by-step 不得拆成两个通用 evidence 命令。已完成 full phase 仅因当前 roster 合法追加新项或 recoverable stale binding 产生 pending 时，mount validator 报 `usable_with_gaps`，允许按 manifest 顺序恢复；结构/基线损坏仍 blocked，Guide 与 `complete-phase` 调用始终严格，第一条重新记录会清除旧完成状态并要求重建 Guide。学生回复“继续/看懂了”只是路由输入，不是完成证据。`teaching_example_roster_exhausted=true`（包括零例题）也只表示该 roster 无待讲项，不能绕过其余完成门禁。
