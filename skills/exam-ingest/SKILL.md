...
- Student-facing output defaults to Simplified Chinese; a persisted `study_state.json` `language` (`English`/`双语`) switches it per exam-cram's dispatch rule (canonical tokens verbatim). The cold-start receipt follows the same dispatch; see [`docs/language-policy.md`](../../docs/language-policy.md).
