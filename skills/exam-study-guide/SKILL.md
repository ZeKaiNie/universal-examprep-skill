...
4. Apply the prompt-image rule before import. `full_prompt` means the original question is already visible in the source image, so omit duplicate OCR/original question text. Show only a translation needed by the active language: if an English prompt image is visible, `zh` and `bilingual` may show its Chinese translation but never repeat the English original; reverse this for a Chinese image. `figure_only` still requires the full original prompt text because the image is only a diagram/table. Never put answer-side images before the worked solution. Preserve but exclude `student_attempt` from prompt and answer inventories; it is not material/official Guide evidence. One occurrence taints the same physical path across quiz, teaching, and every content unit, including safe slash/backslash and Windows case aliases. Reject official use of a tainted path and same-item prompt/answer reuse. Across different items, official prompt/answer reuse remains legal without an attempt; distinct official and attempt paths remain valid.
