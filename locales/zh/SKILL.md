...
- 缺失/旧版/非法 `answer_explanation_mode` 按 `ordinary`：完整教材仍有逐题详细答案解释，但不声称经过第二家模型服务商的隔离调用。`isolated` 仅适用于第二代完整建库，并分两次同意：先说明模型服务商/API 独立计费和保留/隐私边界，取得不上传的规划同意；再对准确规划方案的逐题/图片范围、调用数、按当前价格给出的估算和上传行为作最终同意。不能由 `GPT`、订阅、接口密钥、`full` 或 `visual` 推断。
