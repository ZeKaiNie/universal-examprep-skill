...
- Missing/legacy/invalid `answer_explanation_mode` means `ordinary`: full Guides remain detailed but claim no second-Provider isolation. `isolated` is full-v2-only and needs two consents: Provider/API-billing plus retention/privacy disclosure before no-upload planning, then exact plan-bound item/image scope, count, current-pricing estimate, and upload consent before calls. Never infer it from GPT, subscription, key, full, or visual.
